...
|---------|---------|----------|---------|
| **Animation style** | **11** presets | Hyperspace | Kiểu particle field (thêm **Signal**) |
...
1. **Start:** legacy `message=stream` **hoặc** OpenCode ≥1.15 `service=llm` … `stream`  
2. **End:** `session.idle` / loop end / abort · dual feed stderr + **claimed** log tail  
3. **Tokens:** poll `opencode.db` (`OPENCODE_DB` / `OPENCODE_DATA_DIR`) · multi-process `bridge_pid`
